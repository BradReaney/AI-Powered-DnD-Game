...
const LOCATION_TYPES = [
  { value: "city", label: "City", description: "Towns, villages, and urban settlements" },
  { value: "dungeon", label: "Dungeon", description: "Caves, ruins, and underground complexes" },
  { value: "wilderness", label: "Wilderness", description: "Forests, mountains, and natural areas" },
  { value: "building", label: "Building", description: "Taverns, shops, and specific structures" },
  { value: "other", label: "Other", description: "Unique or special locations" },
]

export function LocationForm({ location, onSave, onCancel, availableLocations = [] }: LocationFormProps) {
  ...
  const addConnection = () => {
    if (selectedConnection && !formData.connections.includes(selectedConnection)) {
      setFormData({
        ...formData,
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
        ...
        <CardDescription>
          {location ? "Update your location details" : "Build a new location for your world"}
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            {formData.inhabitants.length > 0 && (
              <div className="space-y-2">
                <div className="text-sm text-muted-foreground">Current inhabitants:</div>
          ...
          <div className="space-y-4">
            ...
            {availableConnections.length > 0 && (
              <div className="flex gap-2">
                ...
                <Button type="button" onClick={addConnection} size="sm" disabled={!selectedConnection}>
                  <Plus className="h-4 w-4" />
                </Button>
              </div>
            )}
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {formData.connections.map((connectionId) => (
                    <Badge key={connectionId} variant="outline" className="flex items-center gap-1">
