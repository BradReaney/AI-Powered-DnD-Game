...
import { Edit, Heart, Shield, Zap, Sword, Scroll, User, MapPin } from "lucide-react"
          ...
          <CardContent>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-2xl font-bold">{character.hitPoints.current}</span>
                <span className="text-muted-foreground">/ {character.hitPoints.maximum}</span>
              </div>
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-3">
                {Object.entries(character.stats).map(([stat, value]) => (
                  <div key={stat} className="text-center p-4 bg-muted rounded-lg">
                    <div className="text-sm font-medium text-muted-foreground capitalize mb-1">{stat}</div>
                    <div className="text-2xl font-bold mb-1">{value}</div>
                    <div className="text-sm text-muted-foreground">{getModifier(value)}</div>
                  </div>
                ))}
              </div>
            </CardContent>
        ...
        <TabsContent value="skills">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Skills your character is trained in</CardDescription>
            </CardHeader>
            <CardContent>
              {character.skills.length > 0 ? (
                <div className="grid gap-2 md:grid-cols-2">
                  {character.skills.map((skill) => (
                    <Badge key={skill} variant="secondary" className="justify-start">
                      {skill}
                    </Badge>
                  ))}
                </div>
              ) : (
                <p className="text-muted-foreground">No proficient skills selected.</p>
              )}
            </CardContent>
          </Card>
        </TabsContent>
