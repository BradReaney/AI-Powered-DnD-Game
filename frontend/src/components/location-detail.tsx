...
export function LocationDetail({ location, onBack, onEdit, allLocations = [] }: LocationDetailProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Notable people and creatures in this location</CardDescription>
          </CardHeader>
          <CardContent>
            {location.inhabitants && location.inhabitants.length > 0 ? (
              ...
              <p className="text-muted-foreground">No inhabitants listed for this location.</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Places accessible from this location</CardDescription>
          </CardHeader>
          <CardContent>
            {location.connections && location.connections.length > 0 ? (
              <div className="space-y-2">
                {location.connections.map((connectionId) => (
                  <div key={connectionId} className="flex items-center gap-2 p-2 bg-muted rounded">
                    <div className={`p-1 rounded ${getLocationTypeColor(getConnectedLocationType(connectionId))}`}>
              ...
              <p className="text-muted-foreground">This location has no connections to other places.</p>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Ideas for adventures and important details</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="text-center py-8">
            <MapPin className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
            <p className="text-muted-foreground">Adventure hooks and notes coming soon...</p>
          </div>
        </CardContent>
      </Card>
    </div>
