...
export class GeminiClient implements ILLMClient {
  ...
  constructor() {
    ...
    this.flashLiteModel = this.genAI.getGenerativeModel({
      ...
      generationConfig: {
        ...
        topK: 40,
        topP: 0.95,
        maxOutputTokens: 8192, // Increased for better story generation
      },
    });
    ...
    this.flashModel = this.genAI.getGenerativeModel({
      ...
      generationConfig: {
        ...
        topK: 40,
        topP: 0.95,
        maxOutputTokens: 16384, // Increased for better story generation
      },
    });
    ...
    this.proModel = this.genAI.getGenerativeModel({
      ...
      generationConfig: {
        ...
        topK: 40,
        topP: 0.95,
        maxOutputTokens: 32768, // Much higher for Pro model story generation
      },
    });
  ...
  async sendPrompt(request: GeminiRequest): Promise<GeminiResponse> {
    ...
    try {
      ...
      // Try to generate content with selected model, with fallback strategy
      let result;
      try {
        result = await this.generateContent(request, selectedModel);
      } catch (error) {
        // If the selected model fails, try fallback strategy
        if (config.gemini.threeModelFallbackEnabled && !fallbackUsed) {
          ...
          // Try fallback models in order of capability
          const fallbackModels: ('flash-lite' | 'flash' | 'pro')[] = ['flash-lite', 'flash', 'pro'];
          const currentIndex = fallbackModels.indexOf(selectedModel);

          // Remove the failed model and try others
          fallbackModels.splice(currentIndex, 1);

          for (const fallbackModel of fallbackModels) {
            try {
              logger.info('Trying fallback model', { taskId, fallbackModel });
              result = await this.generateContent(request, fallbackModel);
              selectedModel = fallbackModel;
              fallbackUsed = true;
              logger.info('Fallback model succeeded', { taskId, fallbackModel });
              break;
            } catch (fallbackError) {
      ...
      logger.error('Error communicating with Gemini AI:', error);
      ...
      if (errorMessage.includes('429') || errorMessage.includes('quota')) {
        return {
          success: false,
          content:
            'I apologize, but I have reached my daily request limit. Please try again later or consider upgrading your API plan.',
  ...
  private async generateContent(
    ...
    const selectedModel = this.getModelInstance(model);

    if (request.conversationHistory && request.conversationHistory.length > 0) {
      // Use conversation history format for multi-turn chat
      const contents = request.conversationHistory.map(msg => ({
        role: msg.role,
        parts: msg.parts,
      }));

      return await selectedModel.generateContent({ contents });
    } else {
   ...
   */
  private getModelInstance(model: 'flash-lite' | 'flash' | 'pro'): GenerativeModel {
    switch (model) {
      case 'flash-lite':
...
Focus on what's most relevant for continuing the story.`;
  ...
  /**
   * Extract character information from a story response using Gemini 2.0 Flash-Lite
   */
...
Be precise and only extract living beings that can act and interact. Do not extract place names, building names, or location references.`;
  ...
  /**
   * Extract location information from a story response using Gemini 2.0 Flash-Lite
   */
